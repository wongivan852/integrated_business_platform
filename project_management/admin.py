...
class ProjectAdmin(admin.ModelAdmin):
    ...
    def budget_display(self, obj):
        if obj.budget:
            percentage = obj.budget_percentage_used
            if percentage > 100:
                color = 'red'
            elif percentage > 80:
                color = 'orange'
            ...
            return format_html(
                ...
                color, obj.actual_cost, obj.budget
            )
        return '-'
...
class ResourceAdmin(admin.ModelAdmin):
    ...
    def hourly_rate_display(self, obj):
        return format_html('${:,.2f}', obj.hourly_rate)
    hourly_rate_display.short_description = 'Hourly Rate'

    def availability_display(self, obj):
        color = '#28a745' if obj.availability_percentage >= 80 else '#ffc107'
        return format_html(
            '<span style="color: {};">{:.0f}%</span>',
            color, obj.availability_percentage
        )
...
class TaskCostAdmin(admin.ModelAdmin):
    ...
    def planned_display(self, obj):
        return format_html('${:,.2f}', obj.planned_cost)
    ...
    def variance_display(self, obj):
        variance = obj.actual_cost - obj.planned_cost
        if variance > 0:
            color = '#dc3545'
            sign = '+'
        elif variance < 0:
